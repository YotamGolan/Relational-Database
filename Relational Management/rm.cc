...
RC RelationManager::createCatalog()
{
    ...
    // Create tables, columns and indexes tables, return error if any fail
    RC rc;
    ...
    // Add entries for Tables, Columns, and Indexes to Columns table
...
RC RelationManager::deleteTable(const string &tableName)
{
    ...
    // Delete from Indexes table and delete all indexes on table
    rc = rbfm->openFile(getFileName(INDEXES_TABLE_NAME), fileHandle);
    if (rc)
        return rc;

    value = malloc(4 + INDEXES_COL_TABLE_NAME_SIZE);
    ...
    void *data = malloc(INDEXES_COL_ATTR_NAME_SIZE);

    while((rc = rbfm_si.getNextRecord(rid, data)) == SUCCESS)
    {
        string attributeName;
        fromAPI(attributeName, data);
        // Destroy index
...
string RelationManager::getFileName(const string &tableName, const string &attributeName)
{
...
void RelationManager::prepareIndexesRecordData(const string &tableName, const string &attributeName, void *data)
{
    unsigned offset = 0;
    ...
    string file_name = getFileName(tableName);
    int32_t file_name_len = file_name.length();
    ...
    memcpy((char*) data + offset, &file_name_len, VARCHAR_LENGTH_SIZE);
    offset += VARCHAR_LENGTH_SIZE;
    memcpy((char*) data + offset, file_name.c_str(), file_name_len);
    offset += file_name_len;
}
...
RC RelationManager::createIndex(const string &tableName, const string &attributeName)
{
    ...
    // put in helper for getting attribute
    ...
    IndexManager *idxm = IndexManager::instance();
    IXFileHandle ixfileHandle;

    rc = idxm->createFile(getFileName(tableName, attributeName));
    if (rc)
        return rc;

    rc = idxm->openFile(getFileName(tableName, attributeName), ixfileHandle);
    if (rc)
        return rc;
    ...
    rc = rbfm->scan(fileHandle, attrs, attributeName, NO_OP, NULL, projection, rbfm_si);
    if (rc)
        return rc;

    // null bit + attr field
    void* data = malloc(ixAttr.length + 1);
    RID rid;

    while ((rc = rbfm_si.getNextRecord(rid, data)) == SUCCESS)
    {
        // skip records with NULL in attribute field
        if (data == NULL)
            continue;
        ...
        switch (ixAttr.type)
        {
            case TypeInt:
                int32_t integer;
                fromAPI(integer, data);
                rc = idxm->insertEntry(ixfileHandle, ixAttr, &integer, rid);
            break;
            case TypeReal:
                float real;
                fromAPI(real, data);
                rc = idxm->insertEntry(ixfileHandle, ixAttr, &real, rid);
            break;
            case TypeVarChar:
                string str;
                fromAPI(str, data);
                rc = idxm->insertEntry(ixfileHandle, ixAttr, str.c_str(), rid);
            break;
        }
...
{
   IndexManager *idxm = IndexManager::instance();
   RC rc = idxm->destroyFile(getFileName(tableName, attributeName));
   if (rc)
      return rc;
   ...
   void *value = malloc(4 + INDEXES_COL_TABLE_NAME_SIZE);
   int32_t name_len = tableName.length();
   memcpy(value, &name_len, INT_SIZE);
   memcpy((char*)value + INT_SIZE, tableName.c_str(), name_len);

   rc = rbfm->scan(fileHandle, indexDescriptor, INDEXES_COL_TABLE_NAME, EQ_OP, value, projection, rbfm_si);
   if (rc)
       return rc;

   RID rid;
   // null bit + length field + name
   void *data = malloc(INDEXES_COL_ATTR_NAME_SIZE + 5);

   while ((rc = rbfm_si.getNextRecord(rid, data)) == SUCCESS)
   {
      // compare attribute name inside record w/ input
      ...
      {
          // delete record associated w/ index then break
                      ...
                      RM_IndexScanIterator &rm_IndexScanIterator)
{
    ...
    rc = idxm->openFile(getFileName(tableName, attributeName), rm_IndexScanIterator.fileHandle);
    if (rc)
        return rc;

    rc = idxm->scan(rm_IndexScanIterator.fileHandle, ixAttr, lowKey,
                    highKey, lowKeyInclusive, highKeyInclusive, rm_IndexScanIterator.idxm_iter);
...
{
    return idxm_iter.getNextEntry(rid, key);
}
